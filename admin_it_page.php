            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                             <i class="fa fa-fw fa-users"></i>User Manager <small>IT Staff</small>
                        </h1>
				...
				<div class="row">
					<div class="col-lg-10">
						...
						<div class="panel panel-default" id="container-masterlist-it-staff">
								...
								<div class="table-responsive" >
									<table class="table table-hover table-striped tableWithFilter">
										...
										<tbody>
										   ...
										   <?php
											$queryMasterlistStaff = "SELECT * FROM tbl_view_acc_staff ORDER BY it_id ASC";
											$resultMasterlistStaff = mysqli_query($conn,$queryMasterlistStaff) or die (mysqli_error($conn));
											while($rowMasterlistStaff = mysqli_fetch_array($resultMasterlistStaff)){
											echo "<tr>";
											echo "<td> $rowMasterlistStaff[it_id]</td>";
											echo "<td> $rowMasterlistStaff[it_lname]</td>";
											echo "<td> $rowMasterlistStaff[it_fname]</td>";
											echo "<td> $rowMasterlistStaff[it_gender]</td>";
											echo "<td> $rowMasterlistStaff[it_bday]</td>";
											echo "<td>
													<a href='#' class='editStaff'><input type='hidden' name='' class='staffID' value='$rowMasterlistStaff[it_id]' /><i class='fa fa-pencil'></i></a>
													| 
													<a href='#'  class='deleteStaff'><input type='hidden' name='' class='staffID' value='$rowMasterlistStaff[it_id]'> <input type='hidden' name='' class='acctID' value='$rowMasterlistStaff[acct_id]' /><i class='fa fa-trash-o'></i></a>
                        ...
                        <div class="panel panel-default" id="container-addnew-it-staff" style="display:none">
                            <div class="panel-heading">
                                <h3 class="panel-title"><i class="fa fa-plus-circle"></i> Add New IT Staff</h3>
                            </div>
                            <div class="panel-body">
								<form id="staffRegForm" action="javascript:verifyStaffRegfirst()">
								<div id="regPopuptext" class="text-center"></div>
								<input type="hidden" class="form-control" placeholder=" accID" name="accID" id="accID">
					...
					<div class="col-lg-2">
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
								<button class="btn col-lg-12 btn-success" id="btn-masterlist-it-staff">Masterlist</button><br /><br />
								<button class="btn col-lg-12 btn-primary" id="btn-addnew-it-staff">Add New <br /> IT Staff </button><br /><br />
								<!--
				...
				<div id="updateStaffModal" class="modal fade" data-backdrop="static">
				<div class="modal-dialog" >
					<div class="modal-content" style="">
						<div class="modal-header">
							<button type="button" class="close btnClose" data-dismiss="modal" aria-hidden="true">×</button>
							<h3 class="panel-title"><i class="fa fa-plus-circle"></i> Update IT Staff Info</h3>
						</div>
                            <div class="panel-body">
								<form id="updateStaffForm" action="javascript:verifyUpdateStafffirst()">
...
$(".editStaff").click(function(){
	...
	staffIDforUpdate = $(this).children(".staffID").val();
	//alert(staffIDforUpdate);
	$("#updateStaffModal").modal("show");
	getStaffInfo();
});
$(".deleteStaff").click(function(){
	staffIDforUpdate = $(this).children(".staffID").val();
	acctIDforDel = $(this).children(".acctID").val();
	//alert(acctIDforDel);
	//alert(a);
...
$(".deleteStaff").confirm({
    ...
    confirm: function(button) {
		...
		deleteStaff();
		setTimeout(function(){
		location.href = "admin_it_page.php";
		},100);
    },
...
(function($,W,D)
{
    ...
    JQUERY4U.UTIL =
    {
        ...
        {
            //form validation for staffRegForm start
			...
			//form validation staffRegForm end
			
			//form validation form updateStaffForm start
			...
			//form validation form updateStaffForm end
        }
    }
...
//register student START
	...
	var rform = $("#staffRegForm");
	var uform = $("#updateStaffForm");
	function verifyStaffRegfirst(){
		$.getJSON("phpfiles/register_check.php?callback=?",rform.serialize(), function(data)
		{
			...
			{
				$.getJSON("phpfiles/register_it.php?callback=?",rform.serialize(), function(data)
				{
...
//Update IT STAFF START
function verifyUpdateStafffirst(){
	$.getJSON("phpfiles/update_check.php?callback=?",uform.serialize(), function(data)
		{
			...
			}else if(data.uexists =="allow")
			{
				...
				$.getJSON("phpfiles/update_it.php?callback=?",uform.serialize(), function(data)
						{
							//clear form after register success
...
//delete it staff START
function deleteStaff(){
	//alert("testDel" + staffIDforUpdate);
	$.ajax({
		url: "phpfiles/delete_it.php",
		type: "GET",
		data: {"it_id":staffIDforUpdate,"acct_id":acctIDforDel},
		dataType: "json",
...
function getStaffInfo(){
	$.ajax({
		url: "phpfiles/get_info_staff.php",
		type: "GET",
		data: {"it_id":staffIDforUpdate},
		dataType: "json",
		success: function(data){
			//location.href = "admin_students_page.php";
			//alert("success");
			//testname
			$("#updateStaffForm").find('input[name=accID]').val(data[0].acct_id);
			$("#updateStaffForm").find('input[name=accType]').val(data[0].acct_type);
			$("#updateStaffForm").find('input[name=fname]').val(data[0].it_fname);
			$("#updateStaffForm").find('input[name=lname]').val(data[0].it_lname);
			$("#updateStaffForm").find('input[name=mname]').val(data[0].it_mname);
			var $radios = $("#updateStaffForm").find('input:radio[name=gender]');
			if(data[0].it_gender == "Male"){
			$radios.filter("[value='Male']").prop('checked', true);
			}else{
				$radios.filter("[value='Female']").prop('checked', true);
			}
			$("#updateStaffForm").find('input[name=bdate]').val(data[0].it_bday);
			$("#updateStaffForm").find('input[name=username]').val(data[0].username);
			$("#updateStaffForm").find('input[name=password]').val(data[0].password);
			$("#updateStaffForm").find('input[name=cpassword]').val(data[0].password);
